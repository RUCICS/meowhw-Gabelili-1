...
size_t io_blocksize(const char *filename) {
    ...
    size_t fs_blocksize = get_fs_blocksize(filename);
    
    // 确保缓冲区大小是内存页大小和文件系统块大小的公倍数
    // 这里我们选择两者的最小公倍数
    
    // 计算最大值和最小值用于欧几里得算法计算最大公约数
    size_t a = pagesize;
    size_t b = fs_blocksize;
    
    // 如果其中一个值为0，则返回另一个值作为保守选择
    if (a == 0 || b == 0) {
        return (a > b) ? a : b;
    }
    ...
    size_t gcd = a;  // 最大公约数
    size_t lcm = pagesize / gcd * fs_blocksize;  // 最小公倍数
    
    // 为了安全起见，限制最大缓冲区大小为8MB
    // 防止某些情况下得到异常大的缓冲区
    if (lcm > 8 * 1024 * 1024) {
        return 8 * 1024 * 1024;
    }
